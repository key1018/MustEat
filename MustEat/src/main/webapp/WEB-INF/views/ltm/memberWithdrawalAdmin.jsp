<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import = "com.mz.mybatis.common.model.vo.PageInfo, java.util.ArrayList, com.mz.mybatis.member.model.vo.Member" %>
<%
    ...
    <div class="outer">
        ...
        <div class="wrapper-paging">
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination">
					<% if(currentPage != 1) { %>
				    	<li class="page-item"><a class="page-link" href="<%=contextPath%>/memberWithdrawalAdmin.bo?cpage=<%= currentPage-1 %>&a=<%=a%>&c=<%=c%>&search=<%=search%>">&lt;</a></li>
				    <% } %>
				    
				    <% for(int p=startPage; p<=endPage; p++) { %>
				    	
				    	<% if(p == currentPage) { %>
				    		<li class="page-item"><a class="page-link focus" href="<%=contextPath%>/memberWithdrawalAdmin.bo?cpage=<%=p%>&a=<%=a%>&c=<%=c%>&search=<%=search%>"><%= p %></a></li>
				    	<% } else { %>
				    		<li class="page-item"><a class="page-link" href="<%=contextPath%>/memberWithdrawalAdmin.bo?cpage=<%=p%>&a=<%=a%>&c=<%=c%>&search=<%=search%>"><%= p %></a></li>
				    	<% } %>
        ...
        <script>
            ...
            $("select[name=selectbox]").change(function(){
                ...
                switch(val) {
                    case 'no' : location.href="<%=contextPath%>/memberWithdrawalAdmin.bo?cpage=<%=currentPage%>&a=1&c=no&search=<%=search%>";                    
                    break;
                    case 'name' : location.href="<%=contextPath%>/memberWithdrawalAdmin.bo?cpage=<%=currentPage%>&a=2&c=name&search=<%=search%>";
                    break;
            ...
            function search(){
                ...
                location.href="<%=request.getContextPath()%>/memberWithdrawalAdmin.bo?cpage=1&a=<%=a%>&c=<%=c%>&search="+$('#search').val();
            }
