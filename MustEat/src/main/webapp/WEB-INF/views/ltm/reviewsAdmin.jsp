<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import = "com.mz.mybatis.common.model.vo.PageInfo, java.util.ArrayList, com.mz.mybatis.store.model.vo.StoreReview" %>
<%
...
<body>
    ...
    <div class="outer">
        ...
        <p style="display:inline-block; font-size: larger; font-size: x-large;">총 리뷰 : <%=count%> 개</p>
        
        <div style="display:inline; padding-left:55%">
            <button onclick="deleteNo();" class="btn1" style="font-family:'Lucida Sans', 'Lucida Sans Regular', 'Lucida Grande', 'Lucida Sans Unicode', Geneva, Verdana, sans-serif" >리뷰삭제 </button>
        </div>
        ...
        <script>
            ...
            function deleteNo(){
                if(confirm("선택한 리뷰를 삭제하시겠습니까?")){
                    ...
                    //console.log(delArr);
                    $.ajax({
                        url:"<%=request.getContextPath()%>/reviewAdmin2.bo",
                        traditional: true,
                        data:{delArr:delArr},
                        success:function(result){   
                            if(result=='NNN'){
                                location.href="<%=contextPath%>/reviewAdmin.bo?cpage=<%= currentPage %>&a=<%=a%>&c=<%=c%>&search=";
                            }else{
                                alert('bye');
                            }
                        },error:function(){
        ...
        <div class="wrapper-paging">
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination">
					<% if(currentPage != 1) { %>
				    	<li class="page-item"><a class="page-link" href="<%=contextPath%>/reviewAdmin.bo?cpage=<%= currentPage-1 %>&a=<%=a%>&c=<%=c%>&search=<%=search%>">&lt;</a></li>
				    <% } %>
				    
				    <% for(int p=startPage; p<=endPage; p++) { %>
				    	
				    	<% if(p == currentPage) { %>
				    		<li class="page-item"><a class="page-link focus" href="<%=contextPath%>/reviewAdmin.bo?cpage=<%=p%>&a=<%=a%>&c=<%=c%>&search=<%=search%>"><%= p %></a></li>
				    	<% } else { %>
				    		<li class="page-item"><a class="page-link" href="<%=contextPath%>/reviewAdmin.bo?cpage=<%=p%>&a=<%=a%>&c=<%=c%>&search=<%=search%>"><%= p %></a></li>
				    	<% } %>
        ...
        <script>
            ...
            $("select[name=selectbox]").change(function(){
                ...
                switch(val) {
                    case 'no' : location.href="<%=contextPath%>/reviewAdmin.bo?cpage=<%=currentPage%>&a=1&c=no&search=<%=search%>";                    
                    break;
                    case 'name' : location.href="<%=contextPath%>/reviewAdmin.bo?cpage=<%=currentPage%>&a=2&c=name&search=<%=search%>";
                    break;
                    case 'grade' : location.href="<%=contextPath%>/reviewAdmin.bo?cpage=<%=currentPage%>&a=3&c=grade&search=<%=search%>";
                    break; 
			    }
	        });
            ...
            function search(){
                ...
                location.href="<%=request.getContextPath()%>/reviewAdmin.bo?cpage=1&a=<%=a%>&c=<%=c%>&search="+$('#search').val();
            }
