...
<body>
    ...
    <div class="purpleContainer">
      ...
      <c:choose>
       ...
       <img src="${pageContext.servletContext.contextPath}/resources/image/user.png" id="noneIcon">
       	</c:when>
       	<c:when test="${loginUser.memImgPath == '/MustEat/resources/image/user.png' }"> <!-- 기본 이미지로 변경시 -->
       <img src="${pageContext.servletContext.contextPath}/resources/image/user.png" id="noneIcon">
        </c:when>
        <c:otherwise> <!-- 프로필을 변경할 때 (기본이미지가 아닐 떄) -->
        <img src="${loginUser.memImgPath }" id="userIcon">
        </c:otherwise>
      </c:choose>
      
        <div class="userInfo">
          <div class="name">${loginUser.memName }님 반갑습니다!
            <button type="button" class="btn1" onclick="myPageOpen();">내정보</button>
          </div>
          <div class="grade"><strong>등급 : ${loginUser.memGrade } 리뷰어</strong></div>
        </div>
      </div>    
