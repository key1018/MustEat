...
public class OrderStatusListController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 장바구니 담은 '월'만 조회
		ArrayList<Basket> bMonth = new ProductServiceImpl().selectMonth(memId);
		// 장바구니 목록 
		ArrayList<Basket> bList = new ProductServiceImpl().selectBasketList(memId);
		// 주문현황 '월'만 조회
		...
		if (status.equals("preparing")) {
			str = "상품준비중";
			opMonth = new ProductServiceImpl().selectOrderMonth(str, memId);
			opList = new ProductServiceImpl().selectOrderList(str, memId);
		...
		if (status.equals("basket")) { // 장바구니
			request.getRequestDispatcher("WEB-INF/views/key/orderBasketList.jsp").forward(request, response);
		} else if(status.equals("preparing")) { // 상품준비중
			request.getRequestDispatcher("WEB-INF/views/key/orderReadyList.jsp").forward(request, response);
		} else if (status.equals("cancel")) { // 상품취소
			request.getRequestDispatcher("WEB-INF/views/key/orderCancelList.jsp").forward(request, response);
		} else if (status.equals("transit")) { // 배송중
			request.getRequestDispatcher("WEB-INF/views/key/orderInTransitList.jsp").forward(request, response);
		} else if (status.equals("delivered")) { // 배송완료
			request.getRequestDispatcher("WEB-INF/views/key/orderDeliveredList.jsp").forward(request, response);
		}
