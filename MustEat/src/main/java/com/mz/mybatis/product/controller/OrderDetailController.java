...
public class OrderDetailController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		OrderDetail op = new ProductServiceImpl().selectOrderDetail(memId, orderNo);
		ArrayList<OrderDetail> list = new ProductServiceImpl().selectOrderDetailList(memId, orderNo);
		
		request.setAttribute("detail", op);
		request.setAttribute("list", list);
		request.getRequestDispatcher("/WEB-INF/views/key/orderDetail.jsp").forward(request, response);
	}
